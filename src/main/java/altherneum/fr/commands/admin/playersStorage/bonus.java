...
public class bonus implements TabCompleter, CommandExecutor {
                ...
                if (args.length == 4 || args.length == 3) {
                    if (getDataStorage.playerExistForBonus(sender, Bukkit.getOfflinePlayer(args[0]))) {
                        if (args[1].equalsIgnoreCase("add") && args.length == 4) {
                            altherneum.fr.menu.shop.specials.bonus.bonus.addBonus(Bukkit.getOfflinePlayer(args[0]),
                                    altherneum.fr.menu.shop.specials.bonus.bonus.BonusList.valueOf(args[2]),
                                    Integer.valueOf(args[3]));
                            return true;
                        } else if (args[1].equalsIgnoreCase("remove") && args.length == 3) {
                            altherneum.fr.menu.shop.specials.bonus.bonus.removeBonus(Bukkit.getOfflinePlayer(args[0]),
                                    altherneum.fr.menu.shop.specials.bonus.bonus.BonusList.valueOf(args[2]));
                            return true;
                        }
                    }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 1) {
            ...
            for (altherneum.fr.menu.shop.specials.bonus.bonus.BonusList bonus : altherneum.fr.menu.shop.specials.bonus.bonus.BonusList
                    .values()) {
